...
$executa = $db->prepare("UPDATE usuario SET senha=:senha WHERE cod_usuario=:cod_usuario");
$executa->bindParam (':cod_usuario', $_SESSION['cod_usuario']);
$executa->bindParam (':senha', $_post['senha']);


$resultado = $executa->execute();
if ($executa->rowCount()==1) {
	$executa = $db->prepare("UPDATE usuario set login_primeira='2' where cod_usuario=:cod_usuario");
	$executa->bindParam (':cod_usuario', $_SESSION['cod_usuario']);
	$resultado = $executa->execute();
...
header("location: inicio_adm.php.php");
?>
