...
if ($_POST['searchPhrase']!=''){
	$where .= " AND (nome LIKE '%{$_POST['searchPhrase']}%' OR estado LIKE '%{$_POST['searchPhrase']}%' OR cod_professor LIKE '%{$_POST['searchPhrase']}%' )";
}

$get_data['rows'] = array();

$mod_select = $pdo->prepare('SELECT COUNT(*) as total FROM professor WHERE ' . $where);
$mod_select->execute();
