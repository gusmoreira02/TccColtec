...
$q = "SELECT cod_responsavel, nome, rg, cpf, telefone, DATE_FORMAT(data_nasc, '%d/%m/%Y') as data_nasc FROM responsavel WHERE ";

$where = " 1 ";

if ($_POST['searchPhrase']!=''){
	$where .= " AND (nome LIKE '%{$_POST['searchPhrase']}%' OR cpf LIKE '%{$_POST['searchPhrase']}%' OR cod_responsavel LIKE '%{$_POST['searchPhrase']}%' )";
}

$get_data['rows'] = array();

$mod_select = $pdo->prepare('SELECT COUNT(*) as total FROM responsavel WHERE ' . $where);
$mod_select->execute();
