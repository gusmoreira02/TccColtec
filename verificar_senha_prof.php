...
include ("conexao.php");
session_start();
$executa = $db->prepare("SELECT * FROM professor WHERE usuario=:usuario");
...
if ($executa->rowCount()==1){
    ...
    if ($linha->usuario==$_POST['usuario'] AND $linha->senha==$_POST['senha']){
        ...
        if ($linha->login_primeira == "1") {
             header("Location: primeiro_login_form_prof.php?usuario={$linha->usuario} ");
        } 
        else {
            header("Location: menu_professores.php");
        }
        exit;
    }
 else {
    ...
    ?>
    <a class="btn btn-danger" href="login_Prof.html">Voltar</a><br>
